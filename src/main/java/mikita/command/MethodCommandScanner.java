...
public class MethodCommandScanner implements Serializable, Cloneable, Comparable<MethodCommandScanner> {
    ...
    public static final Set<Class<?>> DECIMALS = Set.of(
            float.class, Float.class,
            double.class, Double.class,
            BigDecimal.class
    );
    ...
    @Override
    public MethodCommandScanner clone() throws CloneNotSupportedException {
        return (MethodCommandScanner) super.clone();
    }

    @Override
    public int compareTo(MethodCommandScanner o) {
        String className = commandClass.getName();
        String oClassName = o.commandClass.getName();
        int nameComparation = className.compareTo(oClassName);

        if (nameComparation != 0) {
            return nameComparation;
        }
        if (scanned != o.scanned) {
            return scanned ? 1 : -1;
        }
        return 0;
    }
    ...
    public void scan() {
        List<Method> methods = Arrays.stream(commandClass.getMethods())
    ...
    public Endpoint find(final String[] args) {
        ...
        if(filter.size() == 1) {
            ...
            if(len == 0) {
                if(argsLen == 0) {
                    pair = new Endpoint(method, new Object[0]);
                } else {
                    pair = null;
                }
            } else if(len == argsLen) {
            ...
            BiFunction<Method, BiFunction<Method, String[], Endpoint>, Endpoint> function = (method, bifun) -> {
                String subs = normSubs(method);
                int cutIndex = subs.isEmpty() ? 0 : subs.split(" ").length;
                String[] newArgs = Arrays.copyOfRange(args, cutIndex, args.length);
            ...
            for(Method method : nonLastParamString) {
                Endpoint pair = function.apply(method, MethodCommandScanner::nonLastParamString);
                if(pair == null)
                    continue;
                return pair;
            }
            ...
            for(Method method : lastParamString) {
                Endpoint pair = function.apply(method, MethodCommandScanner::lastParamString);
                if(pair == null)
                    continue;
                return pair;
            }
    ...
    private static Endpoint nonLastParamString(Method method, String[] args) {
        Class<?>[] params = method.getParameterTypes();
        if(params.length == args.length) {
            ...
            return new Endpoint(method, values);
        }
        return null;
    }

    private static Endpoint lastParamString(Method method, String[] args) {
        Class<?>[] params = method.getParameterTypes();
        int len = params.length;
    ...
    private static boolean parseArguments(Class<?>[] params, String[] args, Object[] values, int limit) {
    ...
    private static Object parseArgument(Class<?> param, String arg) throws RuntimeException {
    ...
    private static boolean methodEquals(Method m1, Method m2) {
    ...
    private static String normSubs(Method method) {
    ...
    public boolean equals(Object obj) {
        ...
        if (obj == this)
            return true;
        if (obj instanceof MethodCommandScanner scanner) {
            return commandClass == scanner.commandClass && scanned == scanner.scanned;
        }
        return false;
    }
    ...
    @Override
    public int hashCode() {
        return Objects.hash(commandClass, scanned);
    }

    @Override
    public String toString() {
        return "MethodCommandScanner[commandClass=%s, scanned=%s]".formatted(commandClass.getName(), scanned);
    }
    ...
    public static class MethodScanException extends RuntimeException {
        ...
        public MethodScanException(String message) {
            super(message);
        }

    }

    public record Endpoint(Method method, Object[] args) {
    }
    ...
    static {
        Comparator<Method> comparator = (m1, m2) -> {
            ...
            String classNames1 = String.join(" ", Arrays.stream(Arrays.copyOf(params1, len - 1)).map(Class::getName).toList());
            String classNames2 = String.join(" ", Arrays.stream(Arrays.copyOf(params2, len - 1)).map(Class::getName).toList());

            return classNames1.compareTo(classNames2);
        };

        METHOD_COMPARATOR = comparator.reversed();
    }
